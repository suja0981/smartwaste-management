...
def ingest_telemetry(payload: TelemetryPayload, db: Session = Depends(get_db)):
    ...
    Also feeds data to ML prediction models.
    """
    ...
    # Feed data to ML models (import here to avoid circular dependency)
    try:
        ...
        ml_service.ingest_telemetry(payload.bin_id, telemetry_data)
    except Exception as e:
        # Don't fail telemetry ingestion if ML fails
        print(f"Warning: ML ingestion failed: {e}")
